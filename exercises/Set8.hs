...
union (Shape s1) (Shape s2) = Shape (\p -> s1 p || s2 p)

cut :: Shape -> Shape -> Shape
cut (Shape s1) (Shape s2) = Shape (\p -> (s1 p) && (not $ s2 p))
------------------------------------------------------------------------------
...
paintSolid color (Shape s1) (Picture p1) = Picture p
  where p c = if s1 c then color else p1 c
...
paint (Picture p1) (Shape s1) (Picture p2) = Picture p
  where p c = if s1 c then p1 c else p2 c
------------------------------------------------------------------------------
...
instance Transform Flip where
  apply (FlipX) (Picture pict) = Picture (\(Coord x y) -> pict (Coord (-x) y))
  apply (FlipY) (Picture pict) = Picture (\(Coord x y) -> pict (Coord x (-y)))
