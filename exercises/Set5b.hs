...
treeSize :: Tree a -> Int
treeSize Empty        = 0
treeSize (Node _ l a) = 1 + treeSize l + treeSize a
...
treeMax Empty = 0
treeMax (Node a l b) = max (max a (treeMax l)) (max a (treeMax b))
...
allValues condition (Node a l b) = if (condition a) then (allValues condition l) && (allValues condition b) else False
...
mapTree f (Node a l b) = (Node (f a) (mapTree f l) (mapTree f b))
------------------------------------------------------------------------------
...
cull val (Node a l b) = if a == val then Empty else (Node a (cull val l) (cull val b))
------------------------------------------------------------------------------
...
isOrdered (Node a l b)
    | root && subtrees = True
    | otherwise        = False
    where root = (aV (<a) l) && (aV (>a) b)
          aV c t = allValues c t
          subtrees = isOrdered l && isOrdered b
...
walk s t = if null s then valAtRoot t else if null t then Nothing else if head s == StepL then walk (tail s) (left t) else walk (tail s) (right t)
  where left (Node _ l _) = l
        right (Node _ _ r) = r
        null Empty = True
        null _ = False
...
set s v t = if null t then Empty else if null s then Node v (left t) (right t) else if head s == StepL then Node (root t) (set (tail s) v (left t)) (right t) else Node (root t) (left t) (set (tail s) v (right t))
  where null Empty = True
        null _ = False
        root (Node x _ _) = x
        left (Node _ l _) = l
        right (Node _ _ r) = r
...
search v t = if isEmpty t then Nothing else if root t == v then Just [] else if isJust (leftSearch t) then Just (StepL : fromJust (leftSearch t)) else if isJust (rightSearch t) then Just (StepR : fromJust (rightSearch t)) else Nothing
  where isEmpty Empty = True
        isEmpty _ = False
        root (Node x _ _) = x
        leftSearch (Node _ l _) = search v l
        rightSearch (Node _ _ r) = search v r
        isJust (Just _) = True
