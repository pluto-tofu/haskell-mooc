...
mapMaybe f x = if isNothing x then Nothing else Just (f (fromJust x))
...
mapMaybe2 f x y = if isNothing x || isNothing y then Nothing else Just (f (fromJust x) (fromJust y))
