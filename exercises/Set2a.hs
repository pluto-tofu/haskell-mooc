...
takeFinal :: Int -> [a] -> [a]
takeFinal n list = if length list < n then list else drop (length list - n) xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x list = take i list ++ [x] ++ drop (i + 1) list
...
isPalindrome s = if reverse s == s then True else False
...
greet first maybeLast = "Hello, " ++ first ++ " " ++ maybe "" (\last -> last ++ "!") maybeLast
...
addEithers a b = if isLeft a then a else if isRight a && isRight b then Right (fromRight a + fromRight b) else b
